...
router.post('/register', async (req, res) => {
  try {
    ...
    // Verificar si el email ya existe (buscar en todos los documentos de la colección gimnasios)
...
router.post('/login', async (req, res) => {
  try {
    ...
    // Buscar usuario en todos los documentos de la colección "gyms"
    const allUserDocs = await User.find({});
    let foundUser = null;
    let foundNombreGym = null;

    for (const userDoc of allUserDocs) {
      if (userDoc.users && Array.isArray(userDoc.users)) {
        const user = userDoc.users.find(
          u => u.username === usuario.toLowerCase() && u.activo !== false
        );
        
        if (user) {
          foundUser = user;
          foundNombreGym = userDoc._id; // El _id es el nombreGym
          break;
        }
      }
    }
    ...
    // Obtener información del gimnasio usando el nombreGym
    // Necesitamos buscar en la colección gimnasios por nombreGym
    const allGyms = await Gym.find({});
    let foundGym = null;
    let foundGymId = null;

    for (const gymDoc of allGyms) {
      const docObj = gymDoc.toObject();
      const nombreGymKey = Object.keys(docObj).find(key => key !== '_id' && key !== '__v');
      
      if (nombreGymKey === foundNombreGym) {
        foundGym = docObj[nombreGymKey];
        foundGymId = gymDoc._id;
        break;
      }
    }

    if (!foundGym || !foundGym.activo) {
      return res.status(401).json({ 
    ...
    res.json({
      ...
      gym: {
        gymId: foundGymId,
        nombreGym: foundNombreGym,
        direccion: foundGym.direccion,
        telefono: foundGym.telefono,
