...
router.post('/register', async (req, res) => {
  try {
    ...
    let existingGymId = await Gym.findOne({ gymId });
    ...
    while (existingGymId && counter < 10) {
      ...
      existingGymId = await Gym.findOne({ gymId: finalGymId });
      counter++;
    }
    ...
    if (existingUser) {
      // Si el gym se creó pero el usuario ya existe, eliminar el gym
    ...
    for (const empleado of empleados) {
      // Verificar si el usuario empleado ya existe para este gymId
      const existingEmpleado = await User.findOne({ 
