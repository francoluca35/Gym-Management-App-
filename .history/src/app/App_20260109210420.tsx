...
function App() {
  ...
  const [membersFilter, setMembersFilter] = useState<'expiring-soon' | 'expired' | null>(null);
  ...
  // Limpiar filtro cuando se cambia de tab
  useEffect(() => {
  ...
  const handlePayMembership = async (id: string, months: number) => {
    // Encontrar el miembro para obtener su membresía y precio
    ...
    if (!membership) {
      alert('Error: No se encontró la membresía del miembro.');
      return;
    }

    // Calcular el monto total pagado (precio * meses)
    const amount = membership.price * months;

    const result = await payMembership(id, months, amount);
    if (result.success && result.client) {
      setMembers(members.map(m => m.id === id ? result.client! : m));
      alert(`Cuota pagada exitosamente. La membresía ha sido renovada por ${months} ${months === 1 ? 'mes' : 'meses'}. Total pagado: ${new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS', minimumFractionDigits: 0 }).format(amount)}`);
    } else {
            ...
            <TabsContent value="financial" className="space-y-6">
              <FinancialDashboard 
                ...
                onMembersUpdated={async () => {
                  // Recargar miembros después de borrar datos
                  const currentGymId = gymId || localStorage.getItem('gym_id');
                  if (currentGymId) {
                    const clientsResult = await getClientsGym(currentGymId);
                    if (clientsResult.success && clientsResult.clients) {
