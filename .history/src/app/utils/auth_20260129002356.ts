...
export async function registerGym(data: GymRegistrationData): Promise<{ success: boolean; error?: string; gym_id?: string }> {
  try {
    ...
    if (gimnasiosError) {
      ...
      const msg = gimnasiosError.message
      ...
      return {
        ...
        error: isNetworkError
          ? 'No se pudo conectar a Supabase. Comprueba tu conexión y que el proyecto no esté pausado (dashboard → Restore project).'
          : msg
      }
    }
...
// Login de administrador (sin verificación de IP)
export async function loginAdmin(usuario: string, contraseña: string): Promise<{ success: boolean; error?: string; gymData?: GymData }> {
  try {
    // Buscar en todos los gimnasios activos
    const { data: allGyms, error: gymsError } = await supabase
      .from('gyms')
      .select('gym_id, nombre_gym, users')

    if (gymsError) {
      console.error('Error en consulta gyms:', gymsError)
      return { success: false, error: 'Error al buscar gimnasios. Revisa la consola para más detalles.' }
    }

    if (!allGyms || allGyms.length === 0) {
      return { success: false, error: 'No se encontraron gimnasios registrados' }
    }

    // Buscar el usuario admin en todos los gimnasios
    for (const gym of allGyms) {
      const users: GymUser[] = gym.users || []
      const user = users.find(u => u.usuario === usuario && u.contraseña === contraseña && u.rol === 'admin')
      ...
      if (user) {
        ...
        if (gimnasioError || !gimnasioData) {
          continue // Continuar con el siguiente gimnasio
        }

        // Guardar datos en localStorage (sin verificar IP para admin)
        const ip = await getUserIP()
        localStorage.setItem('gym_ip', ip)
        localStorage.setItem('gym_id', gym.gym_id)
...
// Login de gimnasio (requiere IP correcta - para empleados)
...
export async function checkGymByIP(): Promise<{ exists: boolean; gym_id?: string; nombre_gym?: string }> {
  try {
    ...
    if (gimnasioError) {
      ...
      // Error de red: proyecto pausado o sin conexión
      if (
        gimnasioError.message?.includes('fetch') ||
        gimnasioError.message?.toLowerCase().includes('network') ||
        gimnasioError.message?.toLowerCase().includes('resolved')
      ) {
        console.error(
          'No se pudo conectar a Supabase (ERR_NAME_NOT_RESOLVED). Si usas plan gratuito, el proyecto puede estar pausado: entra en https://supabase.com/dashboard, elige tu proyecto y haz clic en "Restore project". Reinicia el servidor de desarrollo después de cambiar .env.'
        )
      }
