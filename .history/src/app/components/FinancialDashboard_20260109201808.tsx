...
export function FinancialDashboard({ members, memberships, gymId, onMembersUpdated }: FinancialDashboardProps) {
  ...
  const handleDeleteMonthData = async () => {
    ...
    try {
      ...
      // Borrar por cada mes seleccionado
      for (const monthStr of selectedMonths) {
        ...
        // Buscar y borrar clientes con last_payment_date en ese mes
        ...
        if (!error1 && membersWithPayment && membersWithPayment.length > 0) {
          const idsToDelete = membersWithPayment.map(m => m.id);
          ...
          if (!deleteError) {
            ...
            console.error(`Error borrando datos del mes ${month + 1}/${year}:`, deleteError);
          }
        }
        
        // Buscar y borrar clientes con registration_fee_payment_date en ese mes
        ...
        if (!error2 && membersWithRegistration && membersWithRegistration.length > 0) {
          const idsToDelete = membersWithRegistration.map(m => m.id);
          const { error: deleteError } = await supabase
            .from('client_gym')
            .delete()
            .in('id', idsToDelete);
