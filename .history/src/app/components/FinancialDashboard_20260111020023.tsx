...
import { DollarSign, TrendingUp, Users, AlertCircle, TrendingDown, FileText, FileSpreadsheet, Trash2 } from "lucide-react";
...
export function FinancialDashboard({ members, memberships, gymId, onMembersUpdated }: FinancialDashboardProps) {
  ...
  const monthlyData = Array.from({ length: 12 }, (_, i) => {
    ...
    // Contar solo los nuevos miembros registrados en ese mes específico
  ...
  const handleGeneratePDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Ingresos Mensuales: ${formatCurrency(monthlyRevenue)}`, 14, yPos);
    yPos += 7;
    doc.text(`Ingresos Totales Históricos: ${formatCurrency(totalRevenue)}`, 14, yPos);
    yPos += 7;
    doc.text(`Miembros Totales: ${members.length}`, 14, yPos);
    yPos += 7;
    doc.text(`Miembros Activos: ${totalActiveMembers}`, 14, yPos);
    yPos += 7;
    doc.text(`Tasa de Retención: ${retentionRate}%`, 14, yPos);
    yPos += 15;
    
    // Transacciones recientes
    if (allRecentTransactions.length > 0) {
      doc.setFontSize(14);
      doc.text('Transacciones Recientes (últimos 30 días)', 14, yPos);
      yPos += 10;
      
      doc.setFontSize(9);
      doc.text('Miembro', 14, yPos);
      doc.text('Tipo', 70, yPos);
      doc.text('Monto', 110, yPos);
      ...
      allRecentTransactions.slice(0, 20).forEach((transaction) => {
        if (yPos > 280) {
          doc.addPage();
          yPos = 20;
        }
        
        doc.text(transaction.member.substring(0, 30), 14, yPos);
        doc.text(transaction.type, 70, yPos);
        doc.text(formatCurrency(transaction.amount), 110, yPos);
        doc.text(formatDate(transaction.date), 150, yPos);
        yPos += 7;
      });
    }
  ...
  const handleGenerateExcel = () => {
    ...
    const summaryData = [
      ...
      [],
      ['Resumen Financiero'],
      ['Ingresos Mensuales', formatCurrency(monthlyRevenue)],
      ['Ingresos Totales Históricos', formatCurrency(totalRevenue)],
      ['Miembros Totales', members.length],
      ['Miembros Activos', totalActiveMembers],
      ['Miembros Por Vencer', totalExpiringSoon],
      ['Miembros Vencidos', totalExpiredMembers],
      ['Tasa de Retención', `${retentionRate}%`],
      [],
      ['Desglose de Ingresos'],
      ['Ingresos por Membresías (últimos 30 días)', formatCurrency(monthlyRevenueFromMemberships)],
      ['Ingresos por Inscripciones (últimos 30 días)', formatCurrency(monthlyRegistrationRevenue)],
      ['Total Ingresos por Membresías (histórico)', formatCurrency(totalMembershipRevenue)],
      ['Total Ingresos por Inscripciones (histórico)', formatCurrency(totalRegistrationRevenue)],
    ];
    ...
    const transactionsData = [
      ['Miembro', 'Tipo', 'Monto', 'Método de Pago', 'Fecha'],
    ...
    const monthlyDataSheet = [
      ['Mes', 'Ingresos', 'Miembros Activos'],
      ...monthlyData.map(m => [m.month, m.revenue, m.members])
    ];
    
    const ws3 = XLSX.utils.aoa_to_sheet(monthlyDataSheet);
    XLSX.utils.book_append_sheet(wb, ws3, 'Ingresos Mensuales');
    
    // Guardar Excel
    XLSX.writeFile(wb, `reporte-financiero-${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  // Función para borrar datos por mes
  const handleDeleteMonthData = async () => {
    if (!currentGymId) {
      alert('Error: No se pudo identificar el gimnasio.');
      return;
    }

    if (selectedMonths.length === 0) {
      alert('Por favor selecciona al menos un mes para borrar.');
      return;
    }

    if (!confirm(`¿Estás seguro de borrar los datos de ${selectedMonths.length} mes(es) del año ${selectedYear}? Esta acción solo borrará registros de clientes y no se puede deshacer.`)) {
      return;
    }
    ...
    try {
      ...
      // Recolectar todos los IDs a borrar por cada mes seleccionado
      for (const monthStr of selectedMonths) {
        ...
        // Buscar clientes con last_payment_date en ese mes
        const { data: membersWithPayment, error: error1 } = await supabase
          .from('client_gym')
          .select('id')
          .eq('gym_id', currentGymId)
          .gte('last_payment_date', startDateStr)
          .lte('last_payment_date', endDateStr);
        
        if (!error1 && membersWithPayment) {
          membersWithPayment.forEach(m => allIdsToDelete.add(m.id));
        }
        
        // Buscar clientes con registration_fee_payment_date en ese mes
        const { data: membersWithRegistration, error: error2 } = await supabase
          .from('client_gym')
          .select('id')
          .eq('gym_id', currentGymId)
          .not('registration_fee_payment_date', 'is', null)
          .gte('registration_fee_payment_date', startDateStr)
          .lte('registration_fee_payment_date', endDateStr);
        
        if (!error2 && membersWithRegistration) {
          membersWithRegistration.forEach(m => allIdsToDelete.add(m.id));
        }
      }
      
      // Borrar todos los IDs únicos recolectados
      if (allIdsToDelete.size > 0) {
        const idsArray = Array.from(allIdsToDelete);
        // Borrar en lotes para evitar límites de Supabase
        const batchSize = 100;
        for (let i = 0; i < idsArray.length; i += batchSize) {
          ...
          if (deleteError) {
            console.error(`Error borrando lote de datos:`, deleteError);
            throw deleteError;
          }
      ...
      alert(`Se borraron ${deletedCount} registro(s) de ${selectedMonths.length} mes(es) exitosamente.`);
      ...
      alert(`Error al borrar datos: ${error.message || 'Error desconocido'}`);
    } finally {
      setIsDeleting(false);
    }
  };
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Nuevos miembros registrados por mes (últimos 12 meses)</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
