...
export function FinancialDashboard({ members, memberships, gymId, onMembersUpdated }: FinancialDashboardProps) {
  ...
  const handleDeleteMonthData = async () => {
    ...
    if (!confirm(`¿Estás seguro de borrar los datos de ${selectedMonths.length} mes(es) del año ${selectedYear}? Esta acción no se puede deshacer.`)) {
      return;
    }
    ...
    try {
      ...
      // Construir condiciones para cada mes seleccionado
      for (const monthStr of selectedMonths) {
        ...
        // Borrar clientes que tengan last_payment_date o registration_fee_payment_date en ese mes
        // Solo borrar si ambas fechas están en ese mes (o si solo una está)
        const { error } = await supabase
          .from('client_gym')
          .delete()
          .eq('gym_id', currentGymId)
          .or(`and(last_payment_date.gte.${startDateStr},last_payment_date.lte.${endDateStr}),and(registration_fee_payment_date.gte.${startDateStr},registration_fee_payment_date.lte.${endDateStr})`);
        
        if (error) {
          console.error(`Error borrando datos del mes ${month + 1}/${year}:`, error);
        }
      }

      alert(`Datos de ${selectedMonths.length} mes(es) borrados exitosamente.`);
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {isCheckingConnection 
                    ...
                    : dbConnected 
                    ? 'Datos financieros sincronizados correctamente'
